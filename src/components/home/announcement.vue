<template>
<div class="announcement" ref="box">
    <div class="navTop">
        <img src="../../../static/images/common/backWhite.png" class="backWhite" @click="navBack()">
        <h2 class="navTopH2">联盟公告</h2>
    </div>
    <div class="announcementListWrap">
        <mt-loadmore :top-method="loadTop" :bottom-method="loadBottom" @bottom-status-change="handleBottomChange"
            ...
            <div class="announcementList" v-for="(item,index) in newsList" @click="goNewsInfo(item.id)">
                <dl>
                    <dt v-if="item.img_path != ''">{{item.title}}</dt>
                    <dt v-else class="announcementListDt2">{{item.title}}</dt>
...
export default {
    ...
    methods:{
        ...
        // 点击进入详情
        goNewsInfo(id){
            this.$router.push({path: '/newsInfo',query:{id: id, url: "announcement"}});
            // this.$router.push({path: '/newsInfo',query:{id: id, move: "ios"}});
        },
        // 列表展示
        listData() {
            let _this = this
